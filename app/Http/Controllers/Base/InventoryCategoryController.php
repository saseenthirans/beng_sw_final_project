...
class InventoryCategoryController extends Controller
{
    ...
    {
        $category = Category::with(['getCreator',])->orderBy('id','DESC');
        ...
        $data =  Datatables::of($category)
            ...
            ->addColumn('is_home', function ($item) {
                if ($item->is_home == '0') {
            ...
            ->addColumn('action', function ($item) {
                ...
                if (Auth::user()->hasRole('Admin')) {
                    ...
                    $editurl = url('admin/inventory/categories/update/'.Crypt::encrypt($item->id));
                    $logurl = url('admin/inventory/categories/logs/'.Crypt::encrypt($item->id));
                    ...
                    $editurl = url('staff/inventory/categories/update/'.Crypt::encrypt($item->id));
                    return '<a href="' . $editurl . '" class="btn btn-sm btn-primary editbtn square-btn" title="Edit"><svg id="edit_black_24dp" xmlns="http://www.w3.org/2000/svg" width="15.678" height="15.678" viewBox="0 0 15.678 15.678"><path id="Path_783" data-name="Path 783" d="M0,0H15.678V15.678H0Z" fill="none"/><path id="Path_784" data-name="Path 784" d="M3,12.445v1.986a.323.323,0,0,0,.327.327H5.312a.306.306,0,0,0,.229-.1l7.133-7.127-2.45-2.45L3.1,12.21a.321.321,0,0,0-.1.235ZM14.569,5.638a.651.651,0,0,0,0-.921L13.04,3.189a.651.651,0,0,0-.921,0l-1.2,1.2,2.45,2.45,1.2-1.2Z" transform="translate(-1.04 -1.039)" fill="#fff"/></svg></a>';
                }

            })
            ->rawColumns(['status', 'is_home','image','created', 'action','name'])
            ->make(true);
    ...
    public function create($request)
    {
        ...
        $category_banner = '';
        //Category Image
        if(isset($request->category_image) && $request->category_image->getClientOriginalName()){

            $category_image = (new HelperController)->fileUpload($request->category_image,'cat_' ,'inv_category',190,190);
        }

        //Category banner Image
        if(isset($request->banner_image) && $request->banner_image->getClientOriginalName()){

            $category_banner = (new HelperController)->fileUpload($request->banner_image,'cat_banner' ,'inv_category',900,300);
        }
        ...
        $category->banner_image = $category_banner;
        $category->is_home = $request->is_home == true ? 1 : 0;
        $category->created_by = Auth::user()->id;
        $category->save();

        $category_log = new CategoryLog();
        $category_log->category_id = $category->id;
